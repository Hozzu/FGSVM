...
//pkshin start
bool Memory::getDiff(void * dst, void * clean){
  void * gpuMemory = amd::Os::alignedMalloc(owner()->getSize(), MEMOBJ_BASE_ADDR_ALIGN);

  bool result = dev().xferMgr().readBuffer(*this, gpuMemory, amd::Coord3D(0), owner()->getSize(), true);
  if(!result){
    printf("[PK] Fail to read buffer\n");
    return false;
  }

  char * hostMem_char = reinterpret_cast<char *>(gpuMemory);
  char * dst_char = reinterpret_cast<char *>(dst);
  char * clean_char = reinterpret_cast<char *>(clean);
